...
export class Constellation {
    ...
    private async handleRequest(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
        ...
        // Define method handlers in a map to simplify handler selection
        const methodHandlers: Partial<Record<Method, Function>> = {
            [Method.GET]: this.handleGet.bind(this),
            [Method.POST]: this.handlePost.bind(this),
            [Method.DELETE]: this.handleDelete.bind(this)
        };
    
        for (const route of Constellation.routes) {
            const parameters = this.fetchParameters(pathname, route.path);
    
            // Get the handler based on route method
            const handler = methodHandlers[route.method];
            
            if (parameters && handler) {
                // Call the appropriate handler with the parameters and return early
                if (route.method === Method.GET) {
                    handler(route, parameters, pathname, response);
                } else {
                    handler(route, parameters, pathname, request, response);
                }
                return;  // Exit early once a matching route is found
            }
        }
    ...
    private fetchParameters(pathname: string, routePath: string): Record<string, string> | null {
        ...
        console.log("PARAMS: " + urlSegments + " | " + routeSegments);
